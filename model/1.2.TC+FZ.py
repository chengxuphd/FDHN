...
# Notes: This is code for the TC+FZ experiment.
# ===============================
...
class LiarDataset(data.Dataset):
    def __init__(self, data_df, statement, label_onehot, label, subject, speaker, job_title, state_info,
        ...
        self.label = label
        self.metadata = torch.cat((subject.float(), speaker.float(), job_title.float(), state_info.float(), party_affiliation.float(), 
                                   torch.tensor(barely_true_counts, dtype=torch.float).unsqueeze(1), torch.tensor(false_counts, dtype=torch.float).unsqueeze(1), 
                                   ...
                                   torch.tensor(pants_on_fire_counts, dtype=torch.float).unsqueeze(1), context.float()), dim=-1)
    ...
    def __getitem__(self, idx):
        ...
        return statement, label_onehot, label, metadata
...
class LiarModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, n_filters, filter_sizes, output_dim, dropout, padding_idx, input_dim, hidden_dim, n_layers, bidirectional):
        super().__init__()

        self.textcnn = TextCNN(vocab_size, embedding_dim, n_filters, filter_sizes, output_dim, dropout, padding_idx)
        self.cnn_bilstm = CNNBiLSTM(input_dim, embedding_dim, hidden_dim, output_dim, n_layers, bidirectional, dropout)
        self.fuzzy = FuzzyLayer(input_dim, output_dim)
    ...
    def forward(self, text, metadata):
        ...
        metadata_output = self.cnn_bilstm(metadata)
        metadata_output_fuzzy = self.fuzzy(metadata)

        fused_output = self.fuse(torch.cat((text_output, metadata_output, metadata_output_fuzzy), dim=1))
...
def train(num_epochs, model, train_loader, val_loader, optimizer, criterion, model_save):
    ...
    for epoch in range(num_epochs):
        ...
        train_loss = 0.0
        train_accuracy = 0.0
        for statements, label_onehot, label, metadata in train_loader:
        ...
        val_accuracy = 0.0
        with torch.no_grad():
            for statements, label_onehot, label, metadata in val_loader:
                statements = statements.to(DEVICE)
                label_onehot = label_onehot.to(DEVICE)
                label = label.to(DEVICE)
                metadata = metadata.to(DEVICE)

                val_outputs = model(statements, metadata)
...
def test(model, test_loader, model_save):
    ...
    test_accuracy = 0.0
    with torch.no_grad():
        for statements, label_onehot, label, metadata in test_loader:
            statements = statements.to(DEVICE)
            label_onehot = label_onehot.to(DEVICE)
            label = label.to(DEVICE)
            metadata = metadata.to(DEVICE)

            test_outputs = model(statements, metadata)
